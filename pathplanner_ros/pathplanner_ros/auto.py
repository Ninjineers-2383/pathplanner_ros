from .command import Command, SequentialCommandGroup, ParallelCommandGroup
...
class CommandUtil:
    ...
    def _raceGroupFromData(data_json: dict, load_choreo_paths: bool) -> Command:
        ...
        raise NotImplementedError('Race group has not been implemented')

    @staticmethod
    def _deadlineGroupFromData(data_json: dict, load_choreo_paths: bool) -> Command:
        commands = [CommandUtil.commandFromJson(cmd_json, load_choreo_paths) for cmd_json in data_json['commands']]
        # return cmd.deadline(*commands)
        raise NotImplementedError('Deadline group has not been implemented')
...
class AutoBuilder:
    ...
    def configureHolonomic(pose_supplier: Callable[[], Pose2d], reset_pose: Callable[[Pose2d], None],
                           ...
                           config: HolonomicPathFollowerConfig, should_flip_path: Callable[[], bool]) -> None:
        """
    ...
    def pathfindToPoseFlipped(pose: Pose2d, constraints: PathConstraints, goal_end_vel: float = 0.0,
        ...
        :return: A command to pathfind to a given pose
        """
        # TODO!: Need to publish shouldFlipPath as a service
        # return cmd.either(
